...
const Page = async ({
    ...
    const votes = await tabledb.listRows({ databaseId: db, tableId: voteCollection, queries: query });

    votes.rows = await Promise.all(
        votes.rows.map(async vote => {
            const questionOfTypeQuestion =
                vote.type === "question"
                    ? await tabledb.getRow({ databaseId: db, tableId: questionCollection, rowId: vote.typeId, queries: [Query.select(["title"])] })
                    : null;
            ...
            const answer = await tabledb.getRow({ databaseId: db, tableId: answerCollection, rowId: vote.typeId });
